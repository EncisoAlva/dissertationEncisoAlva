...
SVD decomposition in order to compute $W_\rho$ efficiently.
%
...
\begin{equation}
    \ppar{\ppar{\G \AAA_\Theta}} \ppar{\ppar{\G \AAA_\Theta}}^T
    =
...
With these matrices at hand, it is possible to compute $W_\rho$ as
\begin{equation}
    W_\rho = \mathbf{U} \ppar{ \mathbf{\Sigma}^2 + \rho \id_M}^{-1} \mathbf{U}^T
\end{equation}
...
%
Although the stroke occurred in vivo with ECoG monitoring, the TTC stain is only possible post-mortem; the analysis carried out using the ECoG and post-mortem data is, naturally, retrospective}
\end{figure}
