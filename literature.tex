...
From all the estimation methods described on the previous section, all of them converge in a general form
\begin{equation}
\hat{\J}_\Theta = F\ppar{ \Y, \G; \Theta }
\end{equation}
with $\Theta$ a set of parameters that need to be selected beforehand.

One particular case is the linear methods, for which the estimator can be written as
\begin{equation}
\hat{\J}_\Theta
=
\mathbf{K}_{\Theta}\, \Y
...
%
The collection of liner methods include wMNE, FOCUSS, LORETA and their derivates, MSP, among others. 
...
%
The wMNE estimator is used as a reference model since it depends on a single parameter.
%
...
%
Such generalization is not made explicit.
