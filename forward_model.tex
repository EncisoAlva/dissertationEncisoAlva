...
\caption{Decomposition of a dipole with arbitrary momentum as a sum of dipoles whose momentum is parallel to their respective $x-, y-, z-$axis. By definition, a dipole is made of a sink and a source with equal charges. For visual simplicity, only the source is shown; the sink can be constructed similarly}
\label{fig:diagrams3}
\end{figure}

The forward model in equation \eqref{eq:model4} can be extended trivially to account for $T$ EEG measurements in time by setting $\Y \in \R^{M\times T}$, $\Y \in \R^{M\times T}$, and $\G$ unchanged.
%
This is possible due to the quasi-static Maxwell equations. 
%
Since the neural electric fields change in time at relatively low frequencies, they don't induce an important change to the magnetic field [cite Nunez book].
%
