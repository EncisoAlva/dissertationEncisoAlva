...
\caption{A. Cortex surface for the ICBM 152 template, consisting of 15,002 vertices. Views from top, front, left, right (interior), bottom. B. Triangulated surfaces used for the 4-sphere forward model: cortex, inner skull, outer skull, head}
\label{fig:surfaces}
\end{figure}
...
%
Apart from point sources, source patches created using $f_\text{sq}$ are relatively common; their sharp edges facilitate determining the size of the source patch without ambiguity.
%
Some other papers have used $f_\text{Gauss}$ to obtain smooth source patches in space. 
...
Based on informal observations, we consider an SNR level of 20 dB a low-noise condition.
%
...
%
Although it would be optimal to consider many more methods, the large number of trials reduced the number of methods that can be effectively compared.
%
...
%
Figure \ref{fig:results1} displays some of these results.
...
%
In previous sections, we described some mechanisms that could alleviate this effect.
%
...
Table \ref{tab:corr} shows the Pearson correlation of the performance metrics.
%
We can observe that all the metrics seem to be correlated with each other, but none of them correlate with the depth of the source patch.
...
\toprule
      & SD    & AUROC & AP    & Depth  \\
\midrule
DLE   & 0.403 & -0.525 & \phantom{-}0.036  & -0.049 \\
SD    &       & -0.586 & -0.464 & \phantom{-}0.128  \\
AUROC &       &        & \phantom{-}0.524  & -0.114 \\
AP    &       &        &        & -0.094 \\
\bottomrule
\end{tabular}
...
A large group of factors affects the wide variability observed in the results.
%
For instance, figure \ref{fig:deets_SISSY} shows the results for only the SYSSY solver.
%
We can observe that some source patch shapes are recovered more accurately than others.
%
...
This effect was expected since the assumption of minimal norm tends to favor estimations with few high-norm dipoles and many low-norm dipoles.
%
...
%depending on the total variation in space.

%The effect of the source patches is exaggerated for the SISSY estimator since it relies on the penalization of the total variation in space, which 
...
\begin{figure}
    ...
    \caption{Data for this graph is the same from figure \ref{fig:results1} for the sLORETA solver}
...
%Based on the observed results, we must consider an SNR below 10 dB a high-noise condition.
